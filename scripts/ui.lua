    ...
    Callback = function(Value)
        ...
        if Value then
            ...
            if plr.Character then
                ...
                -- Animasyon için vücut ayarları
                if plr.Character:FindFirstChild("UpperTorso") then
                    plr.Character.UpperTorso.Waist.C0 = CFrame.new(0, -0.5, 0) * CFrame.Angles(math.rad(-15), 0, 0)
                end
            end
            ...
            spawn(function()
                ...
                while RealisticBangActive and root and targetRoot do
                    pcall(function()
                        ...
                        -- İleri-geri hareket (sinüs dalgası)
                        local forwardBack = math.sin(timePassed) * bangDepth
                        
                        -- Yukarı-aşağı hareket (cosinüs dalgası)
                        local verticalMove = math.cos(timePassed * 2) * upDownMovement
                        
                        -- Hedefin arka pozisyonu
                        local targetPos = targetRoot.Position + Vector3.new(0, 0.7 + verticalMove, 0)
                        local moveDirection = -targetRoot.CFrame.LookVector
                        
                        -- Son pozisyon
                        local finalPos = targetPos + (moveDirection * (1.5 + forwardBack))
                        
                        -- Vücut rotasyonu (hareketle birlikte değişen)
                        local upperBodyAngle = math.rad(-15 - (math.sin(timePassed) * 10))
                        
                        -- Uygula
                        root.CFrame = CFrame.new(finalPos, targetPos) * CFrame.Angles(math.rad(-5), 0, 0)
                        root.Velocity = Vector3.new(0, 0, 0)
